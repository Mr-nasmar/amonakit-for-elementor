...
class Api {
	...
	public function register_routes() {
        ...
        // Template Library
        // \AmoKitBlocks\Template_Library::instance()->register_routes( $this->namespace );

        // CSS
        \AmoKitBlocks\Manage_Styles::instance()->register_routes( $this->namespace );
    ...
    public function get_category_data( $request ){
        ...
        if ( !isset( $_REQUEST['wpnonce'] ) || !wp_verify_nonce( $_REQUEST['wpnonce'], 'amokit-block-nonce') ){
            return rest_ensure_response([]);
        }

        $data = amokitBlocks_taxnomy_data( $request['querySlug'], $request['queryLimit'], $request['queryOrder'], $request['queryType'] );
    ...
    public function get_image_sizes( $request ){
        ...
        $data = amokitBlocks_get_image_size();
        return rest_ensure_response( $data );
    ...
    public function get_post_data( $request ){
        ...
        $data = [];
        $loop = new \WP_Query( amokitBlocks_Product_Query( $request ) );
        ...
        if( $loop->have_posts() ){
            while( $loop->have_posts() ) {
                ...
                $item['badge']          = [
                    'sale_badge'        => amokit_sale_flash( 'default', false ),
                ];
                ...
                $item['rating']         = [
                    ...
                    'html2'             => amokit_wc_get_rating_html('yes'),
                ];
                ...
                $item['addtocart']      = [
                    ...
                    'text'      => __('Add To Cart','amokit-addons'),
                    'class'     => $cart_btn_class,
                ];
                $item['wishlist']       = [
                    'status' => amokit_has_wishlist_plugin(),
                    'html'   => amokit_add_to_wishlist_button()
                ];
                $item['compare']       = [
                    'status' => amokit_exist_compare_plugin(),
                    'html'   => amokitBlocks_compare_button( array( 'style' => 2 ) ),
                    'html2'   => amokitBlocks_compare_button( array( 'style' => 2, 'btn_text' => '<i class="fa fa-exchange"></i>', 'btn_added_txt' => '<i class="fa fa-exchange"></i>' ) ),
                ];
    ...
    public function get_last_product_data( $request ){
        ...
        // if ( !isset( $_REQUEST['wpnonce'] ) || !wp_verify_nonce( $_REQUEST['wpnonce'], 'amokit-block-nonced') ){
        //     return rest_ensure_response([]);
        // }
        ...
        if( !class_exists('\AmoKit_Default_Data') ) {
            return [];
        }
        
        $product = \AmoKit_Default_Data::instance()->get_product('');
        ...
        $item['addtocart']      = [
            ...
            'html'     => \AmoKit_Default_Data::instance()->default('wl-product-add-to-cart')
        ];

        $item['price_html'] = \AmoKit_Default_Data::instance()->default('wl-single-product-price');
        $item['short_description'] = \AmoKit_Default_Data::instance()->default( 'wl-single-product-short-description' );
        $item['description'] = \AmoKit_Default_Data::instance()->default( 'wl-single-product-description' );
        $item['rating'] = \AmoKit_Default_Data::instance()->default( 'wl-single-product-rating' );
        $item['image'] = \AmoKit_Default_Data::instance()->default( 'wl-single-product-image' );
        $item['meta_info'] = \AmoKit_Default_Data::instance()->default( 'wl-single-product-meta' );
        $item['additional_info'] = \AmoKit_Default_Data::instance()->default( 'wl-product-additional-information' );
        $item['product_tabs'] = \AmoKit_Default_Data::instance()->default( 'wl-product-data-tabs2' );
        $item['product_reviews'] = \AmoKit_Default_Data::instance()->default( 'wl-single-product-reviews' );
        $item['product_stock'] = \AmoKit_Default_Data::instance()->default( 'wl-single-product-stock' );
        $item['product_upsell'] = \AmoKit_Default_Data::instance()->default( 'wl-single-product-upsell' );
        $item['product_related'] = \AmoKit_Default_Data::instance()->default( 'wl-product-related', array('orderby'=>'date','order'=>'desc') );

...
class Blocks_init {
    ...
    public function register_blocks(){
        ...
        foreach ( $block_list as $key => $block ) {
            if( is_array( $block ) ){
                $block_name = str_replace('amokit/', '', trim(preg_replace('/\(.+\)/', '', $block['name'])));
                if( $block['active'] === true && amoKitBlocks_get_option( $key, 'amokit_gutenberg_tabs', 'on' ) === 'on' ){
    ...
    public function prepare_block_data( $block ){
        ...
        $block = apply_filters( 'amokit/register_block_args', $block );

        if ( !function_exists('register_block_type') ) return;

        if( !isset($block['server_side_render']) ) return;

        $block_dir = ( isset($block['is_pro']) && $block['is_pro'] == true && defined( "AMONAKIT_ADDONS_PL_PATH_PRO" ) ) ? AMONAKIT_ADDONS_PL_PATH_PRO . 'blocks/' : AMOKIT_BLOCK_PATH . '/src/blocks/';
        $block_name  = str_replace('amokit/', '', trim(preg_replace('/\(.+\)/', '', $block['name'])));
	...
	public function enqueue_block_assets( $block ){
        ...
        // Assest handle name.
        $handle = 'amokit-' . $block['block_name'];

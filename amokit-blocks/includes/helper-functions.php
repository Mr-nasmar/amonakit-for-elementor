 ...
 * return on/off
 */
function amoKitBlocks_get_option( $option, $section, $default = '' ){
...
*/
function amoKitBlocks_get_last_product_id(){
    global $wpdb;
...
*/
function amoKitBlocks_get_last_order_id(){
...
*/
function amoKitBlocks_edit_mode(){
    ...
    if( !empty( $_GET['post'] ) && $_GET['action'] === 'edit' ){ // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        ...
        if( $post_obj->post_type === 'amokit-template' ) {
            return true;
        }else{
            return false;
        }
 ...
 * @return void
 */
function amoKitBlocks_generate_css( $settings, $attribute, $css_attr, $unit = '', $important = '' ){
 ...
 * @return void
 */
function amoKitBlocks_Dimention_Control( $settings, $attribute, $css_attr, $important = '' ){
    ...
    if( array_key_exists( 'device', $dimensions ) ){
        ...
        foreach( $dimensions as $key => $dimension ){
            ...
            $generate_dimension[$key] = amoKitBlocks_Dimention_Value( $dimension, $css_attr, $important );
        }
        return $generate_dimension;
    }else{
        return amoKitBlocks_Dimention_Value( $dimensions, $css_attr, $important );
    }
 ...
 * @return void
 */
function amoKitBlocks_Dimention_Value( $dimensions, $css_attr, $important ){
 ...
 * @return void
 */
function amoKitBlocks_Background_Control( $settings, $attribute ){
 ...
 */
function amoKitBlocks_is_gutenberg_page() {
 ...
 * current page blocks
 */
function amoKitBlocks_check_inner_blocks( $block ) {
    ...
    if( isset( $current['blockName'] ) && $current['blockName'] != '' ) {
        ...
        if( count( $current['innerBlocks'] ) > 0 ){
            foreach( $current['innerBlocks'] as $innerBlock ) {
                amoKitBlocks_check_inner_blocks( $innerBlock );
            }
        }
    }
 ...
 */
function amoKitBlocks_get_settings_by_blockName( $id, $block_name ){
    $blocks_data = amoKitBlocks_get_blocks( $id );
 ...
 * @return Int
 */
function amoKitBlocks_get_ID(){
    if( class_exists('\AmoKit_Manage_WC_Template') ){
        $post_id = \AmoKit_Manage_WC_Template::instance()->get_builder_template_id();
    }else{
        $post_id = '';
    }
...
/**
 * Check has amoKit block
 *
 * @return bool
 */
function amoKitBlocks_Has_Blocks( $id ){
 ...
 * @return array
 */
function amoKitBlocks_reusable_id( $post_id ){
    $reusable_id = [];
    if( $post_id ){
 ...
 * Get Image Sizes
 */
function amoKitBlocks_get_image_size() {
    $sizes = get_intermediate_image_sizes();
    $filter = [ 'full' => __( 'Full','amokit-addons') ];
 ...
 * @return void
 */
function amoKitBlocks_taxnomy_data( $taxnomySlug = '', $number = 20, $order = 'asc', $type = '' ){
    ...
    if( !empty( $term_data ) && !is_wp_error( $term_data ) ){
        ...
        foreach ( $term_data as $terms ) {
            ...
            // Images
            if( $thumbnail_id ){
                $image_sizes    = amoKitBlocks_get_image_size();
                $image_src      = array();
 ...
 * @return void
 */
function amoKitBlocks_Product_type( $type ) {
    switch ( $type ) {
 ...
 * @return void
 */
function amoKitBlocks_Product_Query( $params ){
    ...
    $query_args['wpnonce'] = wp_create_nonce( 'amokit-block-nonce' );
 ...
 * @return void
 */
function amoKitBlocks_compare_button( $button_arg = array() ){
    ...
    if( class_exists('Ever_Compare') || class_exists('amokit_Ever_Compare') ){

        $button_title       = !empty( $button_arg['title'] ) ? $button_arg['title'] : esc_html__('Add to Compare','amokit-addons');
        $button_text        = !empty( $button_arg['btn_text'] ) ? $button_arg['btn_text'] : esc_html__('Add to Compare','amokit-addons');
        $button_added_text  = !empty( $button_arg['btn_added_txt'] ) ? $button_arg['btn_added_txt'] : esc_html__( 'Product Added','amokit-addons' );

        $comp_link = \EverCompare\Frontend\Manage_Compare::instance()->get_compare_page_url();
        $output = '<a title="'.esc_attr( $button_title ).'" href="'.esc_url( $comp_link ).'" class="amocompare-btn amo-compare" data-added-text="'.esc_attr( $button_added_text ).'" data-product_id="'.esc_attr( $product_id ).'">'.$button_text.'</a>';
        return $output;
        ...
        if( $button_style == 1 ){
            ...
            $output = '<a title="'. esc_attr__('Add to Compare', 'amokit-addons') .'" href="'. esc_url( $comp_link ) .'" class="amo-compare compare" data-product_id="'. esc_attr( $product_id ) .'" rel="nofollow">'.esc_html__( 'Compare', 'amokit-addons' ).'</a>';
        }
        return $output;
    }else{
        return 0;
 ...
 * @return void
 */
function amoKitBlocks_ratting( $ratting_num ){
...
function amoKitBlocks_testimonial_ratting($ratting) {
	return sprintf('<div class="amo-testimonial-ratting">

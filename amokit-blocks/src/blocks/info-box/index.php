<?php
	...
	extract($settings);
	$uniqClass 	 = "amokit-block-". $blockUniqId;
	$classes 	 = [
		$uniqClass,
		"amokit-info-box",
		"amokit-info-box-{$boxStyle}",
		"amokit-grid",
	];
	$columns['desktop'] && $classes[] = "amokit-grid-col-{$columns['desktop']}";
	$columns['tablet'] && $classes[] = "amokit-grid-col-tablet-{$columns['tablet']}";
	$columns['mobile'] && $classes[] = "amokit-grid-col-mobile-{$columns['mobile']}";
	$classNames = implode(" ", $classes);

	$title_shape = $boxStyle === '3' ? '<span class="amo-info-box-item-title-shape"><svg width="54" height="8" viewBox="0 0 67 10" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	$box_shape_svg = $boxStyle === '4' ? '<div class="amo-info-box-item-shape"><svg width="420" height="80" viewBox="0 0 420 80" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		if( is_array( $infoBoxList ) ){
			foreach ( $infoBoxList as $key => $item ) {
				...
				if(empty($item['image']) || empty($item['image']['url'])) {
					$item['image'] = [
						"url" => AMOKIT_BLOCK_URL .'src/assets/images/info-box.png',
						"width" => 150,
						"height" => 150
					];
				}
				...
				$title = !empty($item['title']) ? "<" . tag_escape($titleTag) ." class='amo-info-box-item-title'>
					<a {$link} " . esc_attr($target) . " " . esc_attr($rel) . ">" . esc_html($item['title']) ."</a>
					{$title_shape}
				</" . tag_escape($titleTag) .">" : "";
				$desc = !empty($item['desc']) ? "<p class='amo-info-box-item-desc'>" . esc_html($item['desc']) . "</p>" : "";
				$button = !$hideButton && isset($item['linkText']) && !empty($item['linkText']) ? "<div class='amo-info-box-item-link'><a {$link} " . esc_attr($target) . " " . esc_attr($rel) . ">" . esc_html($item['linkText']) . "</a></div>" : "";

				$infoBoxItem = "<div class='amo-info-box-item'>
					<a class='amo-info-box-item-thumbnail' {$link} " . esc_attr($target) . " " . esc_attr($rel) . ">{$image}</a>
					<div class='amo-info-box-item-content'>
						{$title}
						{$desc}
						{$button}
					</div>

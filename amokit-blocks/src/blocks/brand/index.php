<?php
	...
	$classes 	 = [ $uniqClass, "amokit-brand", "amokit-brand-" . $brandStyle	];
	$slider && $classes[] = "amokit-slick-slider amo-brand-carousel";
	if (!$slider) {
		$classes[] = "amokit-grid";
	...
	$default_image_url = AMOKIT_BLOCK_URL .'src/assets/images/brand.svg';
		...
		<?php
			if( is_array( $brandList ) ){
				foreach ( $brandList as $key => $brand ) {
					...
					echo sprintf('<div class="amo-brand-item"><a %s %s %s>%s</a></div>',

 ...
 * Class.
 */
if ( ! class_exists( 'amokit_Newsletter_Data' ) ) {
    class AmoKit_Newsletter_Data {
        ...
        private function __construct() {
            add_action( 'wp_ajax_amokit_newsletter_subscribe', array( $this, 'process_data' ) );
            add_action( 'wp_ajax_nopriv_amokit_newsletter_subscribe', array( $this, 'process_data' ) );
        }
        ...
        public function process_data() {
            ...
            check_ajax_referer('amokit-admin-ajax-request', 'security');
            ...
            if ( is_email( $email ) ) {
                ...
                if ( ! empty( $data ) ) {
                    ...
                    if ( ! is_wp_error( $request ) ) {
                        $response = array(
                            'status' => 'success',
                            'message' => esc_html__( 'Successfully subscribed.', 'amokit-addons' ),
                        );

                        update_option( 'amokit_newsletter_subscribed', true );
                    } else {
                        $response = array(
                            'status' => 'error',
                            'message' => esc_html__( 'Something went wrong.', 'amokit-addons' ),
                            'request' => $request,
                        );
                    }
                } else {
                    $response = array(
                        'status' => 'error',
                        'message' => esc_html__( 'Invalid data.', 'amokit-addons' ),
                    );
                }
            } else {
                $response = array(
                    'status' => 'error',
                    'message' => esc_html__( 'Invalid email.', 'amokit-addons' ),
                );
            }
        ...
        private function prepare_data( $email = '' ) {
            ...
            $project_pro = 'amokit-pro/amokit_pro.php';
    ...
    AmoKit_Newsletter_Data::get_instance();
}
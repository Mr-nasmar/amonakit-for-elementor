...
if ( ! class_exists( 'NasDesigns_Notices' ) ){
    class NasDesigns_Notices{
         ...
         * @var string
         */
        private static $plugin_domain = 'amokit-addons';
        ...
        public function __construct(){
            ...
            add_action( 'wp_ajax_nasdesigns_notices', [ $this, 'ajax_dismiss' ] );
        }
        ...
        public function ajax_dismiss() {
			...
			if ( ! apply_filters( 'nasdesigns_notice_user_cap_check', current_user_can( $capability ) ) ) {
                $error_message = [
                    'message'  => __('You are not authorized.', 'amokit-addons')
                ];
                wp_send_json_error( $error_message );
			}

            // Nonce verification check
            if( !wp_verify_nonce( $nonce, 'nasdesigns_notices_nonce') ) {
                $error_message = [
                    'message'  => __('Are you cheating?', 'amokit-addons')
                ];
                wp_send_json_error( $error_message );
            }

            if ( ! empty( $notice_id ) && (strpos( $notice_id, 'nasdesigns-notice' ) !== false) ) {
        ...
        public function enqueue_scripts() {
            ...
            $styles = ".nasdesigns-admin-notice.promo-banner {
                position: relative;
                padding-top: 20px !important;
                padding-right: 40px;
            }
            .nasdesigns-admin-notice.notice img, .nasdesigns-review-notice-wrap img{
                width: 100%;
            }
            .nasdesigns-review-notice-wrap{
            ...
            .nasdesigns-review-notice-content {
                margin-left: 15px;
            }
            .nasdesigns-review-notice-action {
                display: flex;
                align-items: center;
                padding-top: 10px;
            }
            .nasdesigns-review-notice-action span.dashicons {
                font-size: 1.4em;
                padding-left: 10px;
            }
            .nasdesigns-review-notice-action a {
                padding-left: 5px;
                text-decoration: none;
            }
            .nasdesigns-review-notice-content h3 {
                margin: 0;
            }";

            $scripts = "jQuery(document).ready( function($) {
                $( '.nasdesigns-admin-notice.is-dismissible' ).on( 'click', '.notice-dismiss,.nasdesigns-notice-close', function(e) {
                    e.preventDefault();
                    let noticeWrap = $( this ).parents( '.nasdesigns-admin-notice' ),
                        ...
                        noticeNonce = '".esc_html( wp_create_nonce( 'nasdesigns_notices_nonce' ) )."';
        ...
        private function prepare_notice( $notice_data ){
            ...
            $classes = [ 'nasdesigns-admin-notice' ];
            ...
            if ( isset( $notice['type'] ) ) {
                ...
                if( $notice['type'] !== 'custom'){
                    ...
                    $notice['dismissible_btn'] = '<button type="button" class="notice-dismiss"><span class="screen-reader-text">'.esc_html__('Dismiss this notice.','amokit-addons').'</span></button>';
                }
            }
            ...
            $notice['id'] = 'nasdesigns-notice-id-' . $notice['id'];

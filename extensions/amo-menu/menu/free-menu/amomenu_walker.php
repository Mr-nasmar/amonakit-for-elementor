...
class AmoKit_Menu_Nav_Walker extends Walker_Nav_Menu {

  public $amomenu_menupos = '';
  public $amomenu_menuwidth = '';
  ...
  function start_lvl( &$output, $depth = 0, $args = array() ) {
        ...
        if ($display_depth == 1) {
         $style = 'style="width:'.$this->amomenu_menuwidth.'px; left:'.$this->amomenu_menupos.'px;"';
        }else{
          $style = '';
        }
  ...
  function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
    ...
    if( isset( $item->menuposition ) && $item->ID  ){
        $this->amomenu_menupos =  $item->menuposition;
    }

    if( isset( $item->menuwidth ) &&  $item->ID ){
         $this->amomenu_menuwidth = $item->menuwidth;
    }
    ...
    if( !empty( $item->classes ) && 
        ...
        if($depth > 0 ){
            $dropdown_icon = '<span class="amomenu-icon"><i class="fas fa-angle-right"></i></span>';
        }else{
            if ( isset( $args->extra_menu_settings['dropdown_icon'] ) ) {
                $dropdown_icon = '<span class="amomenu-icon">'.$args->extra_menu_settings['dropdown_icon'].'</span>';
            } else {
                $dropdown_icon = '<span class="amomenu-icon"><i class="fas fa-angle-down"></i></span>';
            }
        }
    ...
    $item_settings = get_post_meta( $item->ID, 'amokit_menu_settings', true );
    ...
    if( isset( $item->menutag ) && !empty( $item->menutag ) ){
        ...
        $badge = '<span class="amomenu-menu-tag" style="'.$badge_style.'">'.$item->menutag.'</span>';
    }
    ...
    if( !empty( $buildercontent ) ){
        $item_output .= sprintf('<div class="amomenu-content-wrapper sub-menu" style="%1s">%2s</div>', $styles, $buildercontent );
    }

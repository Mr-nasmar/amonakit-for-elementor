;jQuery(document).ready(function($) {
    ...
    var $scrollToTopButton = $('<div class="amo-stt-wrap" id="amokitScrollToTopBtn">'+stt_button_text+ ' ' +icon+ ' </div>').appendTo('body');
    ...
    $(".amo-stt-wrap svg path").css('fill', stt_color);

    // Button hover 
    $('.amo-stt-wrap').on("mouseenter", function() {
        $(this).css("background", stt_bg_color_hover).css('color', stt_color_hover); 
        $(".amo-stt-wrap svg path").css('fill', stt_color_hover);
    ...
    $(window).on("scroll", function() {
        ...
        if ($(this).scrollTop() > 20) {
            $('.amo-stt-wrap').css('visibility', 'visible').css('opacity',1);

        } else {
            $('.amo-stt-wrap').css('visibility', 'hidden').css('opacity',0);
        }
    });

    $('.amo-stt-wrap').on('click', function() {

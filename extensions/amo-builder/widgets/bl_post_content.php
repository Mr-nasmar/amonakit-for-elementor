...
class Bl_Post_Content_ELement extends Widget_Base {
    ...
    public function get_title() {
        return __( 'Post Content', 'amokit-addons' );
    }

    public function get_icon() {
        return 'amokit-icon eicon-post-content';
    }
    ...
    public function get_keywords() {
        return ['post content', 'content', 'post details', 'post description', 'blog content', 'amokit', 'Amona Kit', 'addons'];
    }
    ...
    protected function register_controls() {
        $this->start_controls_section(
            ...
            array(
                'label' => __( 'Post Content', 'amokit-addons' ),
            )
        );
        ...
        $this->start_controls_section(
            ...
            array(
                'label' => __( 'Post Content Style', 'amokit-addons' ),
    ...
    protected function render( $instance = [] ) {
        ...
        if( Elementor::instance()->editor->is_edit_mode() ){
           // echo '<h3>' . __('Post Content', 'amokit-addons' ). '</h3>';
            echo '<p>' . esc_html__('It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using ‘Content here, content here’, making it look like readable English. Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text, and a search for ‘lorem ipsum’ will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by accident, sometimes on purpose injected humour and the like.', 'amokit-addons' ). '</p>';
        }else{
            ...
            // End avoid editor recursion
            if ( amokit_is_elementor_version( '>=', '3.2.0' ) ){
           ...
           if( $build_width ){
                ...
                wp_link_pages( [
                    'before' => '<div class="page-links"><span class="page-links-title elementor-page-links-title">' . __( 'Pages:', 'amokit-addons' ) . '</span>',
                    'after' => '</div>',
                    'link_before' => '<span>',
                    'link_after' => '</span>',
                    'pagelink' => '<span class="screen-reader-text">' . __( 'Page', 'amokit-addons' ) . ' </span>%',

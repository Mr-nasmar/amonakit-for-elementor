			...
			if (!$parameters) {
			  ...
			  if ($http_method == "POST"
				...
				$post_data = OAuthUtil::parse_parameters(
					amokit_get_local_file_data(self::$POST_INPUT)
				);
		...
		class OAuthServer {
		  ...
		  private function get_version(&$request) {
			...
			if ($version !== $this->version) {
				$error_message = sprintf(
					esc_html__('OAuth version \'%s\' not supported', 'amokit-addons'),
					esc_html($version)
				);
		  ...
		  private function get_signature_method(&$request) {
			...
			if (!in_array($signature_method, array_keys($this->signature_methods))) {
				...
				$error_message = sprintf(
					esc_html__('Signature method \'%1$s\' not supported. Try one of the following: %2$s', 'amokit-addons'),
		  ...
		  private function get_token(&$request, $consumer, $token_type="access") {
			...
			if (!$token) {
				// Translate and format the error message
				$error_message = sprintf(
					esc_html__('Invalid %1$s token: %2$s', 'amokit-addons'),
		  ...
		  private function check_signature(&$request, $consumer, $token) {
			...
			if (!$valid_sig) {
			  throw new OAuthException( esc_html__( 'Invalid signature','amokit-addons') );
			}
		  }
		  ...
		  private function check_timestamp($timestamp) {
			if (! $timestamp ) {
				throw new OAuthException(
					esc_html__('Missing timestamp parameter. The parameter is required', 'amokit-addons')
				);
			}
			...
			if (abs($now - $timestamp) > $this->timestamp_threshold) {
				// Translate and format the error message
				$error_message = sprintf(
					esc_html__('Expired timestamp, yours %1$s, ours %2$s', 'amokit-addons'),
		  ...
		  private function check_nonce($consumer, $token, $nonce, $timestamp) {
			if (! $nonce ) {
				throw new OAuthException(
					esc_html__('Missing nonce parameter. The parameter is required', 'amokit-addons')
				);
			}
			...
			if ($found) {
				// Translate and format the error message
				$error_message = sprintf(
					esc_html__('Nonce already used: %s', 'amokit-addons'),

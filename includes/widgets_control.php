...
if ( !class_exists( '\AmoKitOpt\Admin\Options_Field'  ) ) {
    require_once AMOKITOPT_INCLUDES . '/classes/Admin/Options_field.php';
}

class AmoKit_Widgets_Control{
    ...
    public function initiliaze_custom_control($controls_manager){
        if ( file_exists( AMONAKIT_ADDONS_PL_PATH.'admin/include/custom-control/preset-select.php' ) ) {
            $controls_manager->register( new \AmoKit\Preset\Preset_Select());
        }
    }
    // Add custom category.
    public function add_category( $elements_manager ) {
        $elements_manager->add_category(
            'amokit-addons',
            [
                'title' => __( 'Amokit Addons', 'amokit-addons' ),
    ...
    public function init_widgets(){
        ...
        $settings  = \AmoKitOpt\Admin\Options_Field::instance()->get_registered_settings();
        ...
        foreach($widget_list as $option_key => $option){
            ...
            $widget_path = $option['is_pro'] ? AMONAKIT_ADDONS_PL_PATH_PRO : AMONAKIT_ADDONS_PL_PATH;

            if(strpos($option['title'], ' ') !== false){
                $widget_file_name = strtolower(str_replace(' ', '_', $option['title']));
                $widget_class = $option['is_pro'] ? 'amokitPro\Elementor\Widget\AmoKit_'. str_replace(' ', '_', $option['title']).'_Element' : "\Elementor\AmoKit_Elementor_Widget_" . str_replace(' ', '_', $option['title']);
            }else{
                $widget_file_name = strtolower($option['title']);
                $widget_class =$option['is_pro'] ? 'amokitPro\Elementor\Widget\AmoKit_'. $option['title'] .'_Element' : "\Elementor\AmoKit_Elementor_Widget_" . $option['title'];
            }
           
            if(isset($option['third-party-resource'])){
                $widget_status = is_plugin_active($option['third-party-resource']) && ( amokit_get_option( $option_key, $option['option-tab'], $default_status ) === 'on' ) && file_exists( $widget_path.'includes/widgets/amokit_'.$widget_file_name.'.php' ) ? true : false ;
            }else{
                $widget_status = ( amokit_get_option( $option_key, $option['option-tab'], $default_status ) === 'on' ) && file_exists( $widget_path.'includes/widgets/amokit_'.$widget_file_name.'.php' ) ? true : false ;
            }

            if ( $widget_status ){
                if( is_plugin_active('amokit-pro/amokit_pro.php') && file_exists( AMONAKIT_ADDONS_PL_PATH_PRO.'includes/widgets/amokit_'.$widget_file_name.'.php' ) ) {
                    require_once AMONAKIT_ADDONS_PL_PATH_PRO.'includes/widgets/amokit_'.$widget_file_name.'.php';
                } else {
                    require_once $widget_path.'includes/widgets/amokit_'.$widget_file_name.'.php';
                }
    ...
    private function get_widget_list(){
        ...
        $widget_list =[
            'accordion'=> [
                'title' => 'Accordion',
                'option-tab'=>'amokit_element_tabs', 
                'is_pro'   => false,
            ],
            'animatesectiontitle'=> [
            ...
            'bbpress' => [   
                'title' => 'Bbpress',
                'option-tab'=> 'amokit_thirdparty_element_tabs', 
        ...
        return apply_filters( 'amokit_widget_list', $widget_list );
    }
}
AmoKit_Widgets_Control::instance();
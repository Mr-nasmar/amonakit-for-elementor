<?php
namespace Nasdesigns\AmoKit_Builder;
...
class HTRP_Recommended_Plugins {
    ...
    function __construct( $args ) {
        ...
        $this->menu_label        =  !empty( $args['menu_label'] ) ? $args['menu_label'] : esc_html__( 'Recommendations', 'amokit-addons' );
    ...
    public function enqueue_assets( $hook_suffix ) {
        ...
        $localize_vars['buttontxt'] = array(
            ...
            'active'     => esc_html__( 'Activated', 'amokit-addons' ),
        );
                            ...
                            foreach( $plugins as $plugin ):
                                ...
                                if ( ! is_wp_error( $data ) ):
                                    ...
                                    if ( file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && is_plugin_inactive( $data['location'] ) ) {
                                        ...
                                        $button_text    = esc_html__( 'Activate', 'amokit-addons' );

                                    // Not Installed.
                                    } elseif ( ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) ) {

                                        $button_classes = 'button install-now';
                                        $button_text    = esc_html__( 'Install Now', 'amokit-addons' );

                                    // Active.
                                    } else {
                                        $button_classes = 'button disabled';
                                        $button_text    = esc_html__( 'Activated', 'amokit-addons' );
                                    }
                                    ...
                                    <div class="plugin-card htrp-plugin-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
                                        <div class="plugin-card-top">
                                            ...
                                            <div class="desc column-description" style="margin-right: 0;">
                                                ...
                                                <p class="authors">
                                                    <cite><?php echo esc_html__( 'By ', 'amokit-addons' ); ?>
                                        ...
                                        <div class="plugin-card-bottom">
                                            <div class="column-updated">
                                                <?php
                                                    if (! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && $plugins_type == 'pro' ) {
                                                        echo '<a class="button button-primary" href="'.esc_url( $details_link ).'" target="'.esc_attr( $target ).'">'.esc_html__( 'Buy Now', 'amokit-addons' ).'</a>';
                                                    }else{
                                                ?>
                                            ...
                                            <div class="column-downloaded">
                                                <a href="<?php echo esc_url( $details_link ) ?>" target="<?php echo esc_attr( $target ) ?>" <?php echo $modal_class; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><?php echo esc_html__('More Details', 'amokit-addons') ?></a>
                                                <span class="downloaded-count">
                                                    <?php
                                                        if( $plugins_type == 'free' ){
                                                            /* translators: %s: Number of installations. */
                                                            printf( esc_html__( '%s Active Installations', 'amokit-addons' ), $this->active_install_count( $prepare_plugin[$data['slug']]['active_installs'] ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                                                        }
                                                    ?>
                                                </span>
                                            </div>
    ...
    public function active_install_count( $active_installs ){
        ...
        if ( $active_installs >= 1000000 ) {
            ...
            $active_installs_text     = sprintf(
                ...
                _nx( '%s+ Million', '%s+ Million', $active_installs_millions, 'amokit-addons' ),
                number_format_i18n( $active_installs_millions )
            );
        } elseif ( 0 === $active_installs ) {
            $active_installs_text = _x( 'Less Than 10', 'amokit-addons' );
        } else {
    ...
    public function plugin_activation() {
        ...
        if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['location'] ) || ! $_POST['location'] ) {
            wp_send_json_error(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Plugin Not Found', 'amokit-addons' ),
                )
            );
        }
        ...
        wp_send_json_success(
            array(
                'success' => true,
                'message' => esc_html__( 'Plugin Successfully Activated', 'amokit-addons' ),
            )
        );
